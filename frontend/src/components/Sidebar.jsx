...
export default function Sidebar({
  ...
  return (
    <>
      {/* 📱 PORTAL BACKDROP SINK: Closes slide drawer cleanly if user clicks outside sidebar on mobile phones */}
      <style>{`
        @media (max-width: 768px) {
          .responsive-mobile-sidebar {
            ...
            box-shadow: ${isSidebarOpen ? '12px 0 30px rgba(0,0,0,0.6)' : 'none'} !important;
          }
      ...
      `}</style>
      {/* Outside Click Catcher Surface */}
      <div className="mobile-sidebar-backdrop" onClick={toggleSidebar} />
      <aside 
        className={`app-sidebar-pane responsive-mobile-sidebar ${isSidebarOpen ? 'is-expanded' : ''}`} 
        style={{ 
          display: 'flex', 
      ...
      >
        ...
        <div className="sidebar-control-header" style={{ 
          ...
          <button className="sidebar-toggle-trigger" onClick={toggleSidebar} style={{
            ...
            borderRadius: '6px', cursor: 'pointer', padding: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center'
          }}>
            {isSidebarOpen ? '◀' : '▶'}
          </button>       
        ...
        <div className="sidebar-footer-profile-bar" style={{
          ...
          <button 
            ...
            style={{
              ...
              display: 'flex',
              alignItems: 'center',
              justifyContent: (isSidebarOpen || window.innerWidth <= 768) ? 'flex-start' : 'center',
              gap: '10px',
          ...
          >
            ...
            {(isSidebarOpen || window.innerWidth <= 768) && <span style={{ fontWeight: '500' }}>Settings</span>}
          </button>
        </div>
      </aside>
    </>
    ...
    return (
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {isEditing ? (
            <button onClick={(e) => saveRename(e, chat.id)} style={{ background: 'none', border: 'none', color: '#10b981', cursor: 'pointer', padding: '4px' }}>💾</button>
          ) : (
            <>
              <button onClick={(e) => { e.stopPropagation(); onTogglePin(chat.id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8rem', padding: '4px' }}>
                {chat.isPinned ? '📌' : '📍'}
              </button>
              <button onClick={(e) => startEditing(e, chat)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8rem', padding: '4px' }}>✏️</button>
              <button onClick={(e) => { e.stopPropagation(); onDeleteChat(chat.id); }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8rem', padding: '4px' }}>🗑️</button>
            </>
          )}
        </div>
      </li>
